...
function callERPWharehouseStruct(){
	$.ajax({
	  method: "POST",
	  url: "http://dev.net-system.it/includes/io/DataReadExt.php?format=JSON&dbname=default&username=admin&password=258&layoutid=90358",
...
function callERPWharehouseStructPerMaga(maga){
	$.ajax({
		method: "POST",
		url: "http://dev.net-system.it/includes/io/DataReadExt.php?format=JSON&dbname=default&username=admin&password=258&layoutid=90357&datawhere=GRUPPOM='"+maga+"'",
		//data: { name: "John", location: "Boston" }
	})
	.done(function(jsonObj) {
		//alert( "Data Saved: " + jsonObj );

...
class PHPivot{
	...
	public function setKeyROW($rowFieldKey){
        $this->_rows_key = $rowFieldKey;
        return $this;
    }
	...
	//Generates the html code for columns
    protected function getColJSON(&$colpoint, $row_space ='', $coldepth = 0, $isLeftmost = true ){
        $html = '';
		$rows = array();
        if(is_array($colpoint) && count($this->_columns) - $coldepth > 0){
            $new_html = '';
    ...
    public function toArrayJSON(){
		...
		$cols = $this->getColJSON($colpoint);
        for($i = count($this->_rows)-1; $i >= 0 ; $i--){
            array_unshift($cols, $this->_rows[$i]);
        }
		...
		$r = 1;
		if ($rowDepth ==1){
			//$Appo = $row_data['8-Thursday']['ORE']['_val'];
			foreach($this->_table as $row_key => $row_data){
				$AppoRow = array();
				for($i = 0; $i < count($cols); $i++){
					//$Appo[] = $row_data['1-Thursday']['ORE']['_val'];
					if($cols[$i] == $this->_rows[0]){
						$AppoRow[$this->_rows[0]] = $row_key;
					}elseif (isset($row_data[$cols[$i]])){
						$AppoRow[$cols[$i]] = $row_data[$cols[$i]]['ORE']['_val'];
					}else{
						$AppoRow[$cols[$i]] = null;
					}
				}
		...
		if ($rowDepth ==2){
			//$Appo = $row_data['ORD']['8-Thursday']['ORE']['_val'];
			foreach($this->_table as $row_keyA => $row_dataA){
				$AppoRow = array();
				if (is_array($row_dataA)){
					foreach($row_dataA as $row_key => $row_data){
						for($i = 0; $i < count($cols); $i++){
							if($cols[$i] == $this->_rows[0]){
								$AppoRow[$this->_rows[0]] = $row_keyA;
							}elseif($cols[$i] == $this->_rows[1]){
								$AppoRow[$this->_rows[1]] = $row_key;
							}elseif (isset($row_data[$cols[$i]])){
								$AppoRow[$cols[$i]] = $row_data[$cols[$i]]['ORE']['_val'];
							}else{
								$AppoRow[$cols[$i]] = null;
		...
		if ($rowDepth ==3){
			//$Appo = $row_data['ORD']['8-Thursday']['ORE']['_val'];
			foreach($this->_table as $row_keyA => $row_dataA){
				foreach($row_dataA as $row_keyB => $row_dataB){
					$AppoRow = array();
					foreach($row_dataB as $row_key => $row_data){
						for($i = 0; $i < count($cols); $i++){
							if($cols[$i] == $this->_rows[0]){
								$AppoRow[$this->_rows[0]] = $row_keyA;
							}elseif($cols[$i] == $this->_rows[1]){
								$AppoRow[$this->_rows[1]] = $row_keyB;
							}elseif($cols[$i] == $this->_rows[2]){
								$AppoRow[$this->_rows[2]] = $row_key;
							}elseif (isset($row_data[$cols[$i]])){
								$AppoRow[$cols[$i]] = $row_data[$cols[$i]]['ORE']['_val'];
							}else{
								$AppoRow[$cols[$i]] = null;
	...
	protected function ValuesJSON(&$key, &$row, $levels, $type = null){
        $levelshtml = '';
		$rows = array();
        ...
        if(!PHPivot::isDataLevel($row)){
            ...
            foreach($row as $head => $nest){
                if(PHPivot::isSystemField($head)) continue;
                $t = isset($row['_type']) ?  $row['_type'] : null;
                $new_row = $this->ValuesJSON($head, $nest, $levels+1, $t);
                //$html .=  $new_row;
				$rows[] = $new_row;
            }
            if($type == null || strcmp($type, PHPivot::TYPE_ROW) == 0 ){

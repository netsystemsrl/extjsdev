...
function (require, _, util, StyleSheet, Worksheet, SharedStrings, RelationshipManager, Paths, XMLDOM) {
    ...
    _.extend(Workbook.prototype, {
        ...
        /** Added by jsreports */
        setPrintArea: function(startCol, endCol) {
            this.printArea = {
                startCol: startCol,
                endCol: endCol
            };
        ...
        toXML: function () {
            ...
            /** Added by jsreports */
            if (this.printArea) {
                var definedNames = doc.createElement('definedNames');
                var definedName = doc.createElement('definedName');
                definedName.setAttribute('localSheetId', '0');
                definedName.setAttribute('name', '_xlnm.Print_Area');
                definedName.appendChild(doc.createTextNode('Sheet1!$' 
                    + util.positionToLetterRef(this.printArea.startCol, '') 
                    + ':$' + util.positionToLetterRef(this.printArea.endCol, '')));
        ...
        _createWorker: function (requireJsPath, worksheetIndex, callbacks) {
            var worker = new window.Worker(require('./WorksheetExportWorker.js'));
            var self = this;
...
//                console.log("Called back by the worker!\n", event.data);
            ...
            worker.postMessage({
                ...
                config: {
                    paths: {
                        underscore: require('underscore').slice(0, -3)
                    },
                    shim: {
                        'underscore': {
                            exports: '_'

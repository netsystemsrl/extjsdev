...
jsreports.ns('integrations.jasper', jsreports);

jsreports.integrations.jasper.JasperReportDef = function(cfg) {
...
// static method
jsreports.integrations.jasper.JasperReportDef.fromJRXML = function(jrxmlStr) {
    var obj = new jsreports.integrations.jasper.JasperReportDef();
    obj.setJasperDef(jasperDefFromJRXML(jrxmlStr));
    return obj;
};

$.extend(jsreports.integrations.jasper.JasperReportDef.prototype, function() {
...
function getJasperValue(collection, jsrValue) {
    return getMappedValue(collection, 'jsreports', 'jasper', jsrValue);
}

function getJsrValue(collection, jasperValue) {
    return getMappedValue(collection, 'jasper', 'jsreports', jasperValue);
}
...
function get_jasper_element_type(jsr_type) {
    switch (jsr_type) {
        ...
        case 'chart_bar':
            return 'barChart';
        default:
            throw new Error('No known Jasper Reports type for jsreports type: ' + jsr_type);
    }
}
...
return {
    ...
    jsrDefFromJasperDef: function(jasperDef) {
        ...
        var builder = jsreports.createReport(jasperDef._name);
        builder.data('-');
    ...
    /** Load elements from a Jasper report def band into a jsreports ReportBuilder (def).  Def is expected to be pointing at the correct report section. */
    ...
    merge_element_property: function(jasper_el, jsr_el, jsr_prop_name, old_val, new_val) {
        // console.log('merge prop', jsr_prop_name, new_val, old_val)
        if (new_val !== old_val) {
            // console.log('prop changed', jsr_prop_name, old_val, new_val);

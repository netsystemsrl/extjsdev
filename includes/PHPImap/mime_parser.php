...
class mime_parser_class {
    ...
    */
    ...
    Function ResetParserState() {
        ...
        $this->next_token           = '';
        $this->lines                = ($this->track_lines ? array(
            0 => 0
        ) : array());
        $this->line_offset          = 0;
        $this->last_line            = 0;
                    ...
                    switch ($type) {
                        case 'HeaderValue':
                            ...
                            if (!IsSet($decoded['Headers'][$header])) {
                                ...
                                $h                           = 1;
                                $decoded['Headers'][$header] = array(
                                    $decoded['Headers'][$header],
                                    $value
                                );
                                if ($this->extract_addresses && IsSet($this->address_headers[$header]))
                                    $decoded['HeaderPositions'][$header] = array(
            ...
            if ($this->extract_addresses) {
                ...
                for (Reset($headers), $h = 0; $h < $th; Next($headers), ++$h) {
                    ...
                    if (IsSet($this->address_headers[$header]) && $this->address_headers[$header]) {
                        $values = (GetType($headers[$header]) == 'array' ? $headers[$header] : array(
                            $headers[$header]
                        ));
                        $p      = (GetType($positions[$header]) == 'array' ? $positions[$header] : array(
                            $positions[$header]
                        ));
                        $tv     = count($values);
                        for ($v = 0; $v < $tv; ++$v) {
    ...
    */
    ...
    Function CopyAddresses($message, &$results, $header) {
        ...
        if (!IsSet($message['ExtractedAddresses'][$header])) {
            ...
            $values                       = (GetType($message['Headers'][$header]) == 'array' ? $message['Headers'][$header] : array(

...
class Mailbox {
	...
	protected $imapParams = [ 'DISABLE_AUTHENTICATOR' => 'PLAIN'];
	...
	public function getMailHeader($mailId) {
		...
		$header->receiveddatetime = null;
		if (isset($head->date) ){
			$appo = preg_replace('/\(.*?\)/', '', $head->date);
			$header->receiveddatetime = new \DateTime($appo);
			$header->date = new \DateTime($appo);
		}else{
			//echo('ERRORE MSG NO DATE TIME'); 
			//var_dump(imap_headerinfo($imapStream,$mailId));
			//var_dump($date); 
			//DAFARE   A VOLTE NON ESISTE LA DATA NEL HEADER DELL EMAIL
			$appo = preg_replace('/\(.*?\)/', '', date('Y/m/d H:i:s'));
			$header->receiveddatetime = null;
			$header->date = null;
		}

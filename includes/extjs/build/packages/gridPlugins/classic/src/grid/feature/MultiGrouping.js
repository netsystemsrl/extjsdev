...
Ext.define('Ext.grid.feature.MultiGrouping', {
    ...
    requires: [
        'Ext.grid.feature.MultiGroupStore',
        'Ext.grid.column.Groups',
        'Ext.overrides.grid.column.Column',
        'Ext.overrides.grid.header.DropZone'
    ],

    eventPrefix: 'group',

    eventCls: Ext.baseCSSPrefix + 'grid-group-row',
    eventSelector: '.' + Ext.baseCSSPrefix + 'grid-group-row',
    groupSelector: '.' + Ext.baseCSSPrefix + 'grid-group-hd',
    groupSummaryCls: Ext.baseCSSPrefix + 'grid-group-summary',
    groupSummarySelector: '.' + Ext.baseCSSPrefix + 'grid-group-summary',
     ...
     * True to hide the headers that are currently grouped when the grid is rendered for the first time.
     */
     ...
     *  * `'top'`: If the group is expanded or collapsed the summary is shown on the group header
     *  * `'bottom'`: When the group is expanded the summary row is shown as a group footer, after all records/groups are shown
     */
     ...
     * @cfg {String}           groupSummaryTpl.columnName         The column header associated with the field being grouped by *if there is a column for the field*, falls back to the groupField name.
    ...
    // we need this template to fix the recordIndex; it should have a priority bigger than the
    // outerRowTpl from Ext.view.Table
    rowTpl: [
        '{%',
            'var me = this.groupingFeature;',
            'if (!(me.disabled)) {',
    ...
    init: function(grid) {
        ...
        if (lockPartner && lockPartner.dataSource) {
            ...
            me.dataSource = view.dataSource = new Ext.grid.feature.MultiGroupStore({
                multigroupingFeature: me,
                store: store
            });
    ...
    setup: function(values) {
        ...
        for(i = 0; i < length; i++) {
            column = columns[i];
            // if there is a summaryType configured on the column then use that instead of the one from the model
    ...
    showMenuBy: function(clickEvent, t, header) {
        var me = this,
            ...
            groupers = store.getGroupers(),
            headerNotGroupable = header.groupable === false || !header.dataIndex,
            groupMenuMeth = headerNotGroupable ? 'disable' : 'enable',
            isGrouped = store.isGrouped(),
            grouper = groupers.get(header.dataIndex);
    ...
    privates: {
        ...
        getStoreListeners: function () {
            return {
                beforegroupchange: this.onBeforeGroupChange,

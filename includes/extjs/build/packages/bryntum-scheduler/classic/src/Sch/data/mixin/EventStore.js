...
Ext.define("Sch.data.mixin.EventStore", {
    ...
    assignEventToResource : function(event, resource) {
        ...
        if (assignmentStore) {
            ...
            if (Ext.isArray(resource)) resource = resource[0];
    ...
    reassignEventFromResourceToResource : function(event, oldResource, newResource) {
        ...
        var newResourceId = newResource instanceof Sch.model.Resource ? newResource.getId() : newResource; // resource id might be 0 thus we use ? operator
        var oldResourceId = oldResource instanceof Sch.model.Resource ? oldResource.getId() : oldResource; // resource id might be 0 thus we use ? operator

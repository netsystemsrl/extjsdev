 Ext.define('TimerField', {
    extend:'Ext.form.field.ComboBox',
    alias: 'widget.timerfield',
	...
	fromFormattedToSecond: function (OreMinutiSecondi){
		var ore = 0;
		var minuti = 0;
		var secondi = 0;
		if (Object.prototype.toString.call(OreMinutiSecondi) === '[object Date]'){
			ore = OreMinutiSecondi.getHours(); 
			minuti = OreMinutiSecondi.getMinutes();
			secondi = OreMinutiSecondi.getSeconds();
		}else if (OreMinutiSecondi.indexOf(':') != -1){
			OreMinutiSecondi = OreMinutiSecondi.split(':');	
			ore = parseFloat(OreMinutiSecondi[0]); 
			minuti = parseFloat(OreMinutiSecondi[1]);
			secondi = parseFloat(OreMinutiSecondi[2]);
		}else if (OreMinutiSecondi.indexOf('.') != -1){
			OreMinutiSecondi = OreMinutiSecondi.split('.');	
			ore = parseFloat(OreMinutiSecondi[0]); 
			minuti = parseFloat(OreMinutiSecondi[1]);  
		...
		return secondi + (minuti * 60) + (ore * 60 *60);
	},
	fromFormattedToMinute: function (OreMinutiSecondi){
		var ore = 0;
		var minuti = 0;
		var secondi = 0;
		if (Object.prototype.toString.call(OreMinutiSecondi) === '[object Date]'){
		...
		return (secondi /60) + (minuti ) + (ore * 60);
	},
	
    toFormattedFromMinute: function (OreMinuti) {
		var me = this;
		var ore = OreMinuti / (60);
		ore = ore.toFixed();
		if (ore > me.maxHour ) ore = me.maxHour;
		var minuti = (OreMinuti - (ore * 60));
		minuti = minuti.toFixed();
		if (minuti > me.maxMinute ) minuti = me.maxMinute;
		var secondi = 0;
		
		return me.getInitDate(ore ,minuti ,secondi);
    },
    toFormattedFromSecond: function (OreMinutiSecondi) {
		var me = this;
		var ore = OreMinutiSecondi / (60 * 60);
		ore = ore.toFixed();
		if (ore > me.maxHour ) ore = me.maxHour;
		var minuti = (OreMinutiSecondi - (ore * 60 * 60)) / 60;
		minuti = minuti.toFixed();
		if (minuti > me.maxMinute ) minuti = me.maxMinute;
		var secondi = (OreMinutiSecondi - ((ore * 60 * 60) + (minuti * 60)));
    ...
    getSubmitValue: function() {
        ...
        if ( value == null) {
			...
			if (me.submitFormat == 's'){
				value = me.fromFormattedToSecond(value);
			}else if (me.submitFormat == 'i'){
				value = me.fromFormattedToMinute(value);
			}else {
    ...
    setValue: function (v) {
        ...
        if (Ext.isDate(v)) {
			...
			if (format == 's'){
				v = me.toFormattedFromSecond(v);
			}else if (format == 'i'){
				v = me.toFormattedFromMinute(v);
			}else {

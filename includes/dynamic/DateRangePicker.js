 ...
 * Provides a time input field with a time dropdown and automatic time validation.
{
	...
	drpDefaults: {
		...
		//mainBtnIconCls: 'drp-icon-yes',
		mainBtnTextColor: '#ff0000',
		presetPeriodsBtnText: 'Fixed periods',
		//presetPeriodsBtnIconCls: 'drp-icon-yes',
		confirmBtnText: 'Confirm',
		//confirmBtnIconCls: 'drp-icon-calendar',
		bindDateFields: true,
		boundStartField: Ext.getCmp('startDateField'),
		boundEndField: Ext.getCmp('endDateField')
	},
	listeners: {
		menuhide: function(btn) {
		   btn.up('panel').down('displayfield[xRole=info]').setValue (
			   '<b>Period contains</b><br>' + btn.getValue().periodDetailsPrecise.diffAsText + '<br>OR<br>' +
			   btn.getValue().periodDetails.yearsCount + ' years ' + btn.getValue().periodDetails.monthsCount + ' months ' + 
			   btn.getValue().periodDetails.weeksCount + ' weeks ' + btn.getValue().periodDetails.daysCount + ' days ' + 
			   btn.getValue().periodDetails.hoursCount + ' hours ' + btn.getValue().periodDetails.minutesCount + ' minutes ' + 
			   btn.getValue().periodDetails.secondsCount + ' seconds ' + btn.getValue().periodDetails.millisecondsCount + ' milliseconds'
		   );
		}
	}
}
...
Ext.define('Ext.ux.button.DateRangePicker', {
	...
	requires : ['Ext.picker.Date', 'Ext.form.field.Time'],
	menu : {
		plain : true,

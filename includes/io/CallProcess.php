...
//leggi definizione del process	
	if (!IsNullOrEmptyString($ProcessId)){
		...
		if ($rs != false) { 
			if ($rs->RecordCount() > 0) {
				...
				header('Content-Type: application/json');
				echo $Appo;
				WFSendLOG("CallProcess:","Error: ProcessId Not Exist STOP",0,$debugmessage);	
				$conn->close();
				die();
			}
		}else{
			...
			header('Content-Type: application/json');
			echo $Appo;
			WFSendLOG("CallProcess:","Error: ProcessId Not coerente STOP",0,$debugmessage);
			$conn->close();
			die();
		}
		$output["id"] = $ProcessId;
	...
	WFSendLOG("CallProcess:" . $ProcessId ,'Inform: Execute',$execution_time,0,$debugmessage);
...
//leggi definizione da layout
	WFSendLOG("CallProcess:","Inform: **ricerca LayoutId",0,$debugmessage);
...
//Comandi a basso livello
	...
	if ($Source == "NEW")    {
		WFSendLOG("CallProcess:","Inform: STOP",0,$debugmessage);	
		die();
	}
	else if ($Source == "SAVE")   {
		//POST
		WFSendLOG("CallProcess:","Inform: SAVEONPOST",0,$debugmessage);  
		...
		//SAVE
		WFSendLOG("CallProcess:","Inform: SAVECOMMIT",0,$debugmessage);  
		...
		WFSendLOG("CallProcess:","Inform: SAVEAFTER",0,$debugmessage);
		...
		//CLONE
		WFSendLOG("CallProcess:","Inform: CLONE SAVECOMMIT",0,$debugmessage);  
		...
		if (($datasourcetype == 'TABLE')) {
			...
			if ($rs) {
				if ($rs->RecordCount()==1)  {
					...
					WFSendLOG("CallProcess:","Inform: CLONEAFTER",0,$debugmessage);
		...
		WFSendLOG("CallProcess:","Inform: DELETE",0,$debugmessage);
		//WFSetDebug(true);
		WFDELETE(); 
		if (($output["success"] == '') && ($output["failure"] == '')) {$output["success"] = true; $output["failure"] = false;}
		if (($output["failure"] == true) || ($output["success"] == false)) {$output["success"] = false; $output["failure"] = true;}
		$Appo = Array2JSON($output, $debugmessage);
		header("Access-Control-Allow-Origin: *");
		header('Content-Type: application/json');
	...
	//EVAL PROCESS
	WFSendLOG("CallProcess:","**eseguo:" . $Source,0,$debugmessage);
	$ProcessId = '0';	
	try {
		eval($Source);
	} catch (Throwable $e) {
...
//dico cosa dovra aprire il pannello
	if ($ProcessId == '') $ProcessId = '0';
	WFSendLOG("CallProcess:","Inform: ProcessId" . $ProcessId, $execution_time,$debugmessage);
	WFSendLOG("CallProcess:","Inform: messagereturn:" . $output["message"], $execution_time,$debugmessage);
	$output["total"] = 1;
	...
	$output["registrationid"] = $RegistrationId;
	if ($ProcessId != '0'){
		WFSendLOG("CallProcess:","Inform: **riaproNewProcess:", $execution_time,$debugmessage);
		$_POST["layoutid"] =  $LayoutId;
	...
	$execution_time = (microtime(true) - $time_start_process)/60;
	WFSendLOG("CallProcess:","Inform: STOP", $execution_time,$debugmessage);
	$conn->close();	
?>
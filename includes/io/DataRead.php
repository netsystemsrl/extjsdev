<?php	
	...
	WFSendLOG("DataRead:","START",0,$debugmessage);
	...
	$subgrid = false;
	$subgrid = isset($_POST["subgrid"]) ? $_POST["subgrid"] : $subgrid;
	$subgrid = isset($_GET["subgrid"])  ? $_GET["subgrid"]  : $subgrid;
	...
	if (IsNOTNullOrEmptyOrZeroString($objname) && IsNOTNullOrEmptyOrZeroString($LayoutId) && ($objname != 'Form00') ) {
		...
		if ($ObjJson == null) { 
			...
			echo $Appo;
			$conn->close();
			WFSendLOG("DataRead:","ERROR OGGETTO NON TROVATO NEL LAYOUT",0,$debugmessage);
			die();
		}
		...
		if ($subgrid){
			//echo('AAAAAAAAAAAAAAA' . BRCRLF);
			if (isset($ObjJson["subgridlayouteditorid"])	|| array_key_exists("subgridlayouteditorid", $ObjJson)) {
				if (!IsNullOrEmptyString($ObjJson["subgridlayouteditorid"])) $LayoutEditorId = $ObjJson["subgridlayouteditorid"];
				if ($conn->debug==1) echo('<b>obj->subgridLayoutEditorId</b>:' . $LayoutEditorId . BRCRLF);
			}
			
			//db
			if (isset($ObjJson["subgriddisplayfield"])		|| array_key_exists("subgriddisplayfield", $ObjJson)) {
				if (!IsNullOrEmptyString($ObjJson["subgriddisplayfield"])) $displayField = $ObjJson["subgriddisplayfield"];
				if ($conn->debug==1) echo('<b>obj->subgriddisplayfield</b>:' . $displayField . BRCRLF);
			}
			if (isset($ObjJson["subgridvalueField"])		|| array_key_exists("subgridvalueField", $ObjJson)) {
				if (!IsNullOrEmptyString($ObjJson["subgridvalueField"])) $valueField = $ObjJson["subgridvalueField"];
				if ($conn->debug==1) echo('<b>obj->subgridvalueField</b>:' . $valueField . BRCRLF);
			}
			if (isset($ObjJson["subgriddkeyfield"])			|| array_key_exists("subgriddkeyfield", $ObjJson)) {
				if (!IsNullOrEmptyString($ObjJson["subgriddkeyfield"])) $keyField = $ObjJson["subgriddkeyfield"];
				if ($conn->debug==1) echo('<b>obj->subgriddkeyfield</b>:' . $keyField . BRCRLF);
			}
			if (isset($ObjJson["subgriddatasourcefield"])	|| array_key_exists("subgriddatasourcefield", $ObjJson)) {
				if (!IsNullOrEmptyString($ObjJson["subgriddatasourcefield"])) $datasourcefield = $ObjJson["subgriddatasourcefield"];
				if ($conn->debug==1) echo('<b>obj->subgriddatasourcefield</b>:' . $datasourcefield . BRCRLF);
			}
			if (isset($ObjJson["subgriddatasource"])		|| array_key_exists("subgriddatasource", $ObjJson)) {
				if (!IsNullOrEmptyString($ObjJson["subgriddatasource"])) $datasource = $ObjJson["subgriddatasource"];
				if ($conn->debug==1) echo('<b>obj->subgriddatasource</b>:' . $datasource . BRCRLF);
			}
			if (isset($ObjJson["subgridddatasourcetype"])	|| array_key_exists("subgridddatasourcetype", $ObjJson)) {
				if (!IsNullOrEmptyString($ObjJson["subgridddatasourcetype"])) $datasourcetype = $ObjJson["subgridddatasourcetype"];
				if ($conn->debug==1) echo('<b>obj->subgridddatasourcetype</b>:' . $datasourcetype . BRCRLF);
			}
			if (isset($ObjJson["subgridddatasourcedbname"]) || array_key_exists("subgridddatasourcedbname", $ObjJson)) {
				if (!IsNullOrEmptyString($ObjJson["subgridddatasourcedbname"])) $datasourcedbname = $ObjJson["subgridddatasourcedbname"];
				if ($conn->debug==1) echo('<b>obj->subgridddatasourcedbname</b>:' . $datasourcedbname . BRCRLF);
			}
		}
		...
		if (isset($ObjJson["loadAll"])		|| array_key_exists("loadAll", $ObjJson)) {
			if ($conn->debug==1) echo('<b>obj->loadAll</b>:'  . BRCRLF);
		...
		if (($xtypeField == 'dynamicgrid') && ($datasourcefield != '') && ($datawhere == '')){
			if ($conn->debug==1) echo('<b>obj->datawhere SET TO IS NULL </b>:' . $datawhere . BRCRLF);
	...
	if ($datamode == 'add'){
			...
			if (isset($sub["defaultValue"])		|| array_key_exists("defaultValue", $sub)) {
				if ($sub["defaultValue"] != ''){
					...
					try {
						...
						WFSendLOG("DEfaultValue", "Error:" . $sub["defaultValue"] . " " . get_class($e) . ', ' . $e->getMessage() . '.',0,$debugmessage);
					}
		...
		switch ($datasourcetype) {
			...
			case 'SELECT':{
				...
				if (!IsNullOrEmptyString($combowhere)){
					...
					if(($queryField == false) || ($queryField == 'displayField')){
						...
						if (array_key_exists("insertwhere",$ObjJson) == true){
							if ($conn->debug==1) echo('insertwhere');
							$insertWhere = $ObjJson["insertwhere"];
							$insertWhere = ExecFuncInStringSQL($insertWhere);
							if($debugmessage) $output["messagedebug"] = $output["messagedebug"] . ' insertwhere:' . $ObjJson["insertwhere"] . BRCRLF;
							//$sqlwhere[] = "( " . $insertWhere . ")";
						}
					}
				...
				if (IsNullOrEmptyString($datawhere) && $ObjJson) { 
					if (array_key_exists("insertwhere",$ObjJson) == true){
						if ($conn->debug==1) echo('insertwhere');
						$insertWhere = $ObjJson["insertwhere"];
						$insertWhere = ExecFuncInStringSQL($insertWhere);
				...
				if ($combofilterActive){ 
					if (array_key_exists("filterwhere",$ObjJson) == true){
						if ($conn->debug==1) echo('filterwhere');
						$filterWhere = $ObjJson["filterwhere"];
						$filterWhere = ExecFuncInStringSQL($filterWhere);
						if($debugmessage) $output["messagedebug"] = $output["messagedebug"] . ' filterWhere:' . $ObjJson["filterwhere"] . BRCRLF;
						
						$sqlwhere[] = "( " . $filterWhere . ")";
					}
				}
					...
					foreach ($appoggio as $sub) {
						if ($sub["property"] == 'innerSearch'){
							...
							foreach($CollectObjList as $subObj) {
								$obj = ReturnOnObjectPropertyValue($LayoutEditorJson, "name", $subObj["name"]);
								if (array_key_exists("datasourcetype",$obj) == true){
									if ( ($obj["datasourcetype"] == 'SELECT') && (array_key_exists("valueField",$obj) == true) ) {
										$sqlSubGrid = "SELECT a." . $obj["valueField"] . " 
														FROM (" . ExecFuncInStringSQL($obj["datasource"]) . ") a 
				...
				//COSTRUZIONE SQL AGGIUNGO WHERE e ORDER
				...
				//DEBUG SQL
				if($debugmessage) $output["messagedebug"] = $output["messagedebug"] . "DEBUG SQL: " . $sql;
				if ($conn->debug==1) echo("<b>BOOKMARK</b>:DEBUG SQL" . $sql . BRCRLF);
				//LANCIO SQL
				try {   
					//$rs = $conn->Execute($sql);
					if ($conn->debug==1) {
				...
				// DATA
				if ($conn->debug==1) echo("<b>BOOKMARK</b>:FETCH DATA" . BRCRLF);
				$RecordCountResult = 0;
				if ($modeldef === true){
						...
						while (!$rs->EOF){							
							...
							for ($i=0; $i < $ColumnCountResult; $i++) {
								...
								if (($rs->fields[$camponome] == '') || ($rs->fields[$camponome] == null)){
									if ($conn->debug==1) echo("<b>BOOKMARK</b>:campi Normali NULL" . BRCRLF);
									$keys[$camponome] = null;
								}
								elseif ($output["fields"][$i]["typephp"] == 'datetime'){
									if ($conn->debug==1) echo("<b>BOOKMARK</b>:campi Normali DATETIME" . BRCRLF);
								...
								elseif (($RecordLimit != 1) && (($campoxtype == "combobox") || ($campoxtype == "dynamiccombo") || ($campoxtype == "dynamictreecombo"))) {
									if ($conn->debug==1) echo("<b>BOOKMARK</b>:campi Normali COMBOBOX" . BRCRLF);
									$keys[$camponome] = null;
									...
									if (array_key_exists("displayField", $output["columns"][$i]["editor"])){
										...
										switch ($output["columns"][$i]["editor"]["datasourcetype"]) {
											...
											case 'TREE' :
											case 'SELECT':{
												if ($conn->debug==1) echo("<b>BOOKMARK</b>:campi Normali COMBOBOX SELECT" . BRCRLF);
												if (!IsNullOrEmptyString($rs->fields[$i])){ 
													...
													if (IsNumeric($rs->fields[$i])){
														if ($conn->debug==1) echo("<b>BOOKMARK</b>:campi Normali COMBOBOX SELECT NUMERIC" . BRCRLF);
														...
														if ($conn->debug==1) echo("<b>BOOKMARK</b>:campi Normali COMBOBOX SELECT STRING" . BRCRLF);
														$keys[$camponome] = DLookup($conn,
																				$output["columns"][$i]["editor"]["displayField"],
																				$output["columns"][$i]["editor"]["datasource"],
																				$output["columns"][$i]["editor"]["valueField"] . " = " . $conn->qStr($rs->fields[$i]) );
													}
												}
											}
											break;
											case 'PSV':{
												if ($conn->debug==1) echo("<b>BOOKMARK</b>:campi Normali COMBOBOX PSV" . BRCRLF);
												$output["columns"][$i]["editor"]["datasource"] = str_replace('|',';',$output["columns"][$i]["editor"]["datasource"]);
											}
											case 'CSV':{
												if ($conn->debug==1) echo("<b>BOOKMARK</b>:campi Normali COMBOBOX CSV" . BRCRLF);
											...
											break;
											case 'PSV2':{
												if ($conn->debug==1) echo("<b>BOOKMARK</b>:campi Normali COMBOBOX PSV2" . BRCRLF);
												$output["columns"][$i]["editor"]["datasource"] = str_replace('|',';',$output["columns"][$i]["editor"]["datasource"]);
											}
											case 'CSV2':{
												if ($conn->debug==1) echo("<b>BOOKMARK</b>:campi Normali COMBOBOX CSV2" . BRCRLF);
												if (IsNOTNullOrEmptyOrZeroString($rs->fields[$i])){ 
													$output["columns"][$i]["editor"]["datasource"] = str_replace(',',';',$output["columns"][$i]["editor"]["datasource"]);
											...
											break;
											default:{
												if ($conn->debug==1) echo("<b>BOOKMARK</b>:campi Normali COMBOBOX ELSE" . BRCRLF);
												$keys[$camponome] =  $rs->fields[$i];
											}
											break;
										}
								...
								elseif ((stripos($camponome, 'JSON') == true) && ( $RecordLimit == 1 )){
									if ($conn->debug==1) echo("<b>BOOKMARK</b>:campi Normali JSON" . BRCRLF);
									$keys[$camponome] = $rs->fields[$camponome];
								...
								elseif (($campoxtype == 'textarea') && ($RecordLimit != 1)) {
									if ($conn->debug==1) echo("<b>BOOKMARK</b>:campi Normali textarea" . BRCRLF);
								...
								elseif (($campoxtype == 'dynamicimage')){
									if ($conn->debug==1) echo("<b>BOOKMARK</b>:campi Normali dynamicimage" . BRCRLF);
									$keys[$camponome] = $rs->fields[$camponome];
								}
								elseif ($campoxtype == 'checkcolumn'){
									if ($conn->debug==1) echo("<b>BOOKMARK</b>:campi Normali checkcolumn" . BRCRLF);
									$keys[$camponome] = $rs->fields[$camponome];
									...
									if ($conn->debug==1) echo("<b>BOOKMARK</b>:campi Normali ELSE" . BRCRLF);
							...
							foreach($LayoutObjList as $sub) {
								if (($sub["name"] != $sub["datasourcefield"]) && !IsNullOrEmptyString($sub["datasourcefield"])){
									...
									if (substr($datasourcefield,0,1) == "=") {
										...
										WFSendLOG("DataRead", 'Eval ($datasourcefield = '. $datasourcefield . ");",0,$debugmessage);
										...
										try {
											...
											$datasourcefield = '';
											//WFSendLOG("DataRead", "Error:" . get_class($e) . ', ' . $e->getMessage() . '.',0,$debugmessage);
										}
				...
				if ($conn->debug==1) echo("<b>BOOKMARK</b>:ORDER IF OBJECT IS COMBO" . BRCRLF);
			...
			break;
			case 'PROCEDURE':{
				if ($conn->debug==1) echo('PROCEDURE');
				$sql =  "CALL " . $datasource . "";
				$procedure = $conn->PrepareSP($sql);
				//LANCIO SQL
				try {   
					//$rs = $conn->Execute($sql);
					if ($conn->debug==1) {
						$start_time = microtime(true); 
						$rs = $conn->execute($procedure);
						echo 'Timer' . (microtime(true) - $start_time) . BRCRLF;
					}else{
						$rs = $conn->execute($procedure);
					}
			...
			break;
			case 'CODE':{
				WFSendLOG("DataRead", 'Eval (' . $datasource . ';);',0,$debugmessage);
			...
			break;
			case 'PROC' :{
				WFSendLOG("DataRead", 'Proc (' . $datasource . ';);',0,$debugmessage);
				...
				if ($rs != false) { 
					if ($rs->RecordCount() > 0) {
						...
						header('Content-Type: application/json');
						echo $Appo;
						WFSendLOG("DataRead:","Error: ProcessId Not Exist STOP",0,$debugmessage);
						$conn->close();
						die();
					}
				}else{
					...
					header('Content-Type: application/json');
					echo $Appo;
					WFSendLOG("DataRead:","Error: ProcessId Not coerente STOP",0,$debugmessage);
					$conn->close();
					die();
				}
	...
	/*ORDER IF OBJECT IS COMBO (ON DECODED)
	$ToOrderData = object_clone($output["columns"]);
	unset($output["columns"]);
	$ToOrderDirection = SORT_ASC;
	$dataToOrder = sortArrayByKey($ToOrderData, 'name',true,SORT_ASC);
	$output["columns"] = $ToOrderData;
	*/
	...
	$execution_time = (microtime(true) - $start_timeDatRead)/60;
	WFSendLOG("DataRead:","STOP",$execution_time,$debugmessage);

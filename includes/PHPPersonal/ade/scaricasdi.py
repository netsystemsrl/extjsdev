...
print('Creo Directory' + path)
if not os.path.exists(path):
    os.makedirs(path)
    
with open(path + '/' + 'fe_ricevute.json', 'wb') as f:
    f.write(r.content)
    
print('Inizio a scaricare le fatture')
with open(path + '/' + 'fe_ricevute.json') as data_file:    
    ...
    for fattura in data['fatture']:
        ...
        if r.status_code == 200:
            ...
            #informazioni_associate_4225372506.xml
            serial = fname[0]
            serial = serial.split('.')[0]
            serial = serial.split('_').pop()
            print('Downloading metadati per ' + serial)
        ...
        if r.status_code == 200:
            d = r.headers['content-disposition']
            fname = re.findall("filename=(.+)", d)
            fnamedef = serial + '_' + fname[0]
            print('Downloading ' + fnamedef)
            with open(path + '/' + fnamedef, 'wb') as f:
                f.write(r.content)

	...
	$writer->startElement('FatturaElettronicaHeader');
		...
		$writer->startElement('CedentePrestatore');
			...
			$writer->startElement('Sede');
				$writer->writeElement('Indirizzo', $AnagraficaAzienda['INDIRIZZO'] . $AnagraficaAzienda['INDIRIZZO2']);
		...
		$writer->startElement('CessionarioCommittente');
			$writer->startElement('DatiAnagrafici');
				...
				$writer->startElement('Anagrafica');
					$writer->writeElement('Denominazione', $AnagraficaCLIFAT['DESCRIZIONE']); 
				$writer->endElement(); 
			$writer->endElement();  
			
			$writer->startElement('Sede');
				$writer->writeElement('Indirizzo', $AnagraficaCLIFAT['INDIRIZZO'] . $AnagraficaCLIFAT['INDIRIZZO2']); 
...
function SDIDecodeFatturaXML($FileName = '', $FatID = null) {
	...
	if ((IsNOTNullOrEmptyOrZeroString($IdFiscaleIVA->IdCodice->__toString())) && (IsNOTNullOrEmptyOrZeroString($DatiAnagrafici->CodiceFiscale->__toString())) ) {
		$AnagraficaCLIFAT = WFVALUEDLOOKUP('*', 'anagrafiche', "PIVA = '"  . $IdFiscaleIVA->IdCodice->__toString() . "' 
	...
	if (($AnagraficaCLIFAT == '' ) && (IsNOTNullOrEmptyOrZeroString($DatiAnagrafici->CodiceFiscale->__toString())) ){
		$AnagraficaCLIFAT = WFVALUEDLOOKUP('*', 'anagrafiche', "CF = '" . $DatiAnagrafici->CodiceFiscale->__toString() . "'");
	}
	...
	if ($AnagraficaCLIFAT == '') {
		...
		if (property_exists ($Anagrafica , 'Denominazione' )){
			...
			$pos = strrpos($AnagraficaCLIFAT['DESCRIZIONE'], $Anagrafica->Nome->__toString() . ' ' . $Anagrafica->Cognome->__toString());
			if ($pos === false) { 
				$AppoRecord['DESCRIZIONE'] = $Anagrafica->Nome->__toString() . ' ' . $Anagrafica->Cognome->__toString();
			}
		}
	...
	foreach ($DettaglioLinee as $DettaglioLinea){
		...
		if (property_exists ( $CodiceArticolo , 'CodiceValore' )){
			$ArticoloListino = WFVALUEDLOOKUP('*','articolilistini',"CT_ANAGRAFICHE = " . $AnagraficaCLIFAT['ID'] . 

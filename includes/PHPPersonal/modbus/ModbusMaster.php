...
class ModbusMaster
{
    ...
    public $status;
    public $timeout_sec = 2; // Timeout 5 sec
	...
	function readWord($address = 339){
		//32bit signed Little Endian Byteswap
		try {
			$recData = $this->readMultipleRegisters($this->moduleId, $address, 2);
	...
	function readDoubleWord($address = 339){
		//64bit signed Little Endian Byteswap
		try {
			$recData = $this->readMultipleRegisters($this->moduleId, $address, 4);
			//return unpack("L",pack("C*",$recData[1],$recData[0]));
    ...
    function RxBOOL($Rx, $Ofs)
    {
        return (ord($Rx[$Ofs]));
    }
    // Funzioni alias per conversione dati da funzione "ReadMultipleRegisters".
    function RxUSINT($Rx, $Ofs)
    {
        return (self::RxBYTE($Rx, $Ofs));
    }
    function RxUINT($Rx, $Ofs)
    {
        return (self::RxWORD($Rx, $Ofs));
    }
    function RxUDINT($Rx, $Ofs)
    {
        return (self::RxDWORD($Rx, $Ofs));
    }
    // Funzioni per conversione dati da funzione "ReadMultipleRegisters".
    function RxBYTE($Rx, $Ofs)
    {
        if (!$this->Endianness) return (ord($Rx[$Ofs + 1]));
        else return (ord($Rx[$Ofs]));
    }
    function RxWORD($Rx, $Ofs)
    {
        return ((ord($Rx[$Ofs]) * 256) + ord($Rx[$Ofs + 1]));
    }
    function RxDWORD($Rx, $Ofs)
    {
        return (self::RxEndian(substr($Rx, $Ofs, 4)));
    }
    function RxREAL($Rx, $Ofs)
    {
        $Pk = pack("L", self::RxEndian(substr($Rx, $Ofs, 4)));
        $Uk = unpack("f", $Pk);
        return ($Uk[1]);
    }
    function RxSTRING($Rx, $Ofs)
    {
        for ($Str = "";$Ofs < 256;$Ofs += 2)
        {
            ...
            if (!$this->Endianness)
            {
                if (ord($As[1])) $Str .= $As[1];
                else return ($Str); //Carico MSB
                if (ord($As[0])) $Str .= $As[0];
                else return ($Str); //Carico LSB
    ...
    private function readMultipleRegistersPacketBuilder($unitId, $reference, $quantity)
    {
        ...
        $buffer3 = '';
        //$buffer3 .= iecType::iecINT(rand(0,65000));   // transaction ID
        $buffer3 .= iecType::iecINT(1); // transaction ID
        $buffer3 .= iecType::iecINT(0); // protocol ID
        $buffer3 .= iecType::iecINT($dataLen + 1); // lenght
        $buffer3 .= iecType::iecBYTE($unitId); //unit ID
        // return packet string
    ...
    private function readWriteRegistersParser($packet)
    {
        $data = array();
        // if not exception
        if (!$this->responseCode($packet)) return false;
        // get data
        for ($i = 0;$i < ord($packet[8]);$i++)
        {
            $data[$i] = ord($packet[9 + $i]);

...
class PHPAnviz {
    ...
    private function parseResponse($response, $type) {
        ...
        if ($type == 1) {
            foreach ($response as $res) {
                ...
                $json['ch'] = $this->hexdecprotected(implode(array_slice($resArr, 1, 4)));
                $json['ack'] = $this->hexdecprotected(implode(array_slice($resArr, 5, 1)));
                $json['ret'] = implode(array_slice($resArr, 6, 1));
                $json['len'] = $this->hexdecprotected(implode(array_slice($resArr, 7, 2)));
    ...
    public function downloadTARecords($type = PHPAnviz::DOWNLOAD_NEW) {
		...
		if (is_array($recordInfo) == false ) return [];
    ...
    private function fixName($name) {
        ...
        while (strlen($newName) < 40) {
			if ($i+1 > $maxi) {
				...
				$newName = $newName . '00' . $name[$i] . $name[$i + 1];
			}
			$i += 2;
        }
        return $newName;
    ...
    function uploadStaffInfo($users) {
        ...
        elseif ($res['ret'] == PHPAnviz::ACK_FULL && $res['ack'] == 0xF3) {
            return 'user full';
        }
        else{
            return $res['ret'];
    ...
    public function getDeviceId() {
        ...
        if ($res['ret'] == PHPAnviz::ACK_SUCCESS) {
            $data = $this->hexdecprotected(implode($res['data']));
            return $data;
        }
        return false;
    }
    ...
    public function hexdecprotected($appo){
        if (ctype_xdigit($appo)){
            $data = hexdec($appo);
        }else{
            $data = false;
        }
